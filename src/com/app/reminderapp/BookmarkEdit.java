...
public class BookmarkEdit extends Activity {
    ...
    private void populateFields() {
        if (mRowId != null) {
        	...
        	// clicked
        	Cursor bookmark = bmarkDbHelper.fetchLocation(mRowId);
    ...
    private void performAction(Action action) {
    	...
    	switch( action ){
    		...
    		case DELETE:    		
    			if( mRowId != null){
    				bmarkDbHelper.deleteLocation(mRowId);
    			}
    			setResult(RESULT_OK);
    			finish();
    			break;
		...
		public void run() {
		  	try{    	        
    	        ...
    	        if (mRowId == null) { // new reminder
    	        	long id = bmarkDbHelper.createLocation(bookmark);
    	            if (id > 0) {
    	            ...
    	            if(bmarkDbHelper.updateLocation(mRowId, bookmark)){
    	            	result = ResultCode.SAVED;
    	            }else{
    	            	result = ResultCode.SAVE_ERROR;
    	            }
